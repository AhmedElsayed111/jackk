...
▀▀███▀▀▀██▄           ██               ██               █████████
  ...
  ███    ███ ███          ██               ██             ██                   ██
▄█████████▀           ██               ██             ██                    ██▄████████▀ │VICTOR ﹏ @elbob_x
│تم تطوير وبرمجة السورس من قبل البوب│@elbob_x
#-------------------------------------------------------------------
]]
...
Victor = Token:match("(%d+)")
redis:set(Victor..":VERSION",1)
redis:set(Victor..":SUDO_ID:",SUDO_USER)
redis:set(Victor..":DataCenter:",'German')
redis:set(Victor..":UserNameBot:",BOT_User)
redis:set(Victor..":NameBot:",BOT_NAME)
redis:hset(Victor..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3Victor_INSTALL","Yes")
info = {} 
...
Victor = Token:match("(%d+)")
our_id = tonumber(Victor)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(Victor..":UserNameBot:")
SUDO_ID = tonumber(redis:get(Victor..":SUDO_ID:"))
SUDO_USER = redis:hgetall(Victor..'username:'..SUDO_ID).username
version = redis:get(Victor..":VERSION")
DataCenter = redis:get(Victor..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..Victor..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(Victor..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(Victor..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(Victor..'group:add'..msg.chat_id_) then 
msg.GroupActive = true
else
msg.GroupActive = false
end

if msg.sender_user_id_ == 798474922 then 
msg.TheRank =  'مطور السورس'
msg.Rank = 1

elseif msg.sender_user_id_ == 1289997596 then 
msg.TheRank =  'مطور السورس'
msg.Rank = 1
...
elseif redis:sismember(Victor..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(Victor..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(Victor..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(Victor..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(Victor..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(Victor..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(Victor..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(Victor..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(Victor..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(Victor..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(Victor..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(Victor..':NameBot:')..[[ .
...
if msg.GroupActive and redis:get(Victor..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(Victor..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(Victor..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(Victor..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.Victor then 
if msg.text and ScriptFile.iVictor then
for k, Victor in pairs(ScriptFile.Victor) do
local SearchText = msg.text:match(Victor)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.iVictor(msg,{msg.text:match(Victor)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",Victor.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dVictor then
if ScriptFile.dVictor(msg) == false then
return false
end
...
if Plug.Victor then 
if msg.text and Plug.iVictor then
for k, Victor in pairs(Plug.Victor) do
local SearchText = msg.text:match(Victor)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(Victor..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(Victor..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.iVictor(msg,{msg.text:match(Victor)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",Victor.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dVictor then
Plug.dVictor(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source Victor")
end 

end
else
print("The File Script.lua Not Runing in The Source Victor")
...
function tdcli_update_callback(data)
	...
	redis:srem(Victor..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/Victor/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/Victor/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/Victor/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/Victor/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/Victor/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/AhmedElsayed111/Victor/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(Victor..":SUDO_ID:")
redis:hset(Victor..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(Victor..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(Victor..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(Victor..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(Victor..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(Victor..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(Victor..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(Victor..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(Victor..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(Victor..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(Victor..'group:add'..data.chat_.id_) then
	redis:set(Victor..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(Victor..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(Victor..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(Victor..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	Victor..'group:name'..k,IDS.Title,
	Victor..'num_msg_Victor'..k,5,
	Victor..'group:add'..k,true,
	Victor..'lock_link'..k,true,
	Victor..'lock_spam'..k,true,
	Victor..'lock_webpage'..k,true,
	Victor..'lock_markdown'..k,true,
	Victor..'lock_flood'..k,true,
	Victor..'lock_bots'..k,true,
	Victor..'mute_forward'..k,true,
	Victor..'mute_contact'..k,true,
	Victor..'mute_document'..k,true,
	Victor..'mute_inline'..k,true,
	Victor..'lock_username'..k,true,
	Victor..'replay'..k,true
	)
	redis:sadd(Victor..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(Victor..'username:'..ID,'username',user)
	redis:sadd(Victor..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(Victor..'username:'..ID,'username',user)
	redis:sadd(Victor..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(Victor..'username:'..ID,'username',user)
	redis:sadd(Victor..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(Victor..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(Victor..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(Victor..'username:'..data.user_.id_).username
	...
	redis:hset(Victor..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
